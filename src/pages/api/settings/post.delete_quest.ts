        ...
        await session.withTransaction(async () => {
            const [update_result, log_result] = await Promise.all([
                configCollection.updateOne(
                    { config_type: 'game_onetime_quests', [`quests.${quest_id}`]: { $exists: true } },
                    { $unset: {[`quests.${quest_id}`]: 1 } },
                    { session }
                ),
                dashLogCollection.insertOne({ log_type: 'quest/delete', tele_id: req.tele_user.tele_id, quest_id, created_at: new Date() }, { session }),
            ]);

            if (update_result.modifiedCount === 0 || log_result.acknowledged !== true) throw new Error('Failed to update quest.');
        });
